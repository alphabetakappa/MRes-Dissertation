...
Both these methods are a viable way of modelling complex systems and have both been used for a long time in a large number of fields [REFS???]. %REFS????
...
Creating this transformative method has a number of important in two aspects, increasing the acceptance of agent-based models and creating a model for viewing systems in two perspectives. Increasing the acceptance of agent-based models has a number of benefits including: 
\begin{itemize}
...
To achieve this transformation between a set of recurrence relations and a agent-based model, three steps need to be undertaken, designing the step-by-step transformation between the two models, creating an automated process to do these steps, and proving the correctness of each step. This papers scope is the first step, designing the step-by-step transformation, the second step is trivial after the first albeit with a large time commitment.\\
Undertaking this first step poses a large number of conceptual challenges, including:
\begin{itemize}
...
%lots of abm and used a lot just not by ecconmiists 

%dont mention proof or conversion, or say the conversions is not part of this project but has been parital done any way 
...
Though this example deals with a very physical system, emergence may be created due to many different form of communication between entities. Such as verbal and visual communication in social systems, which can lead to  emergent behaviour including the break down in social cooperation~\cite{socialemrgence}.\\
Looking at the financial markets one can see how they represent a interaction based system, with a large number of independent or semi-independent entities interacting with each other through different methods, such as market trades.\\
One prominent type of emergence that can occur in an interaction dynamic based system, are feedback loops. Feedback loops are where the input information to an entity is in some way dependent on the output information of that same entity, normally from a previous moment in time. In their simplest form this can just be a single entity supplying an input to its self, shown in Fig.~\ref{fig:exampleselffeedback}.\\
...
Since feedback loops can be destabilising and hence damage the system in which they occur, there is a large amount of interest from researchers interested in regulation on the understanding of these loops with in the financial markets.  
...
\begin{itemize}
   \item what is emergent behaviour? 
   \item it is a fair assumption that emergent behaviour also occurs in the financial markets
   \item how this can occur, well feedback loops
   \item feedback loops can be damaging this leads to the interest of researcher in regulation, looking for ways to stop feedback loops that cause large monetary lose, and in extreme cases damage the economy.
   \item an example of Behaviour of this kind are flash crashes.
   \item a well known flash crash is the one 2010, give an explanation of this crash
\end{itemize}
