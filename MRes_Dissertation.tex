...
%help me write the sim function for InterDyne so I can show it
...
%should i move lite review into background? though doesnt make as much sense there 
...
%%%%%%%%%%rewrite
%rewrite section about automated translation in more detail 

%rewrite subsections of translation 

%%%%%%%%%%finish

%write conversion code. Lexer, parser, first step
...
%add harness explanation to background (major) (chris help) 

%add about turing completeness to Method for Achieving a Two-Views Approach

%add about interacting recurrence relations to RecurrencesRelations


%in numeric testing add a table with the input values tested and the outputs for each step
...
%fix code so it is all shown (not running of the page like currently) (major) 

%dont say paper, say project or dissertation

%get rid of phrase two-views 

%change code colouring to not coloured 
...
This model however has difficulties in design as it can be challenges in creating a large continuous description of a system containing many components, also the function calls as a method of information passing can make dynamically tracking the flow of information during a simulation difficult~\cite{rrbtrack}.   
%write this instead
% however, the problem of tracking individual behaviour and determining particular characteristics to each element still exists. 
