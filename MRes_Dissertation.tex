...
%%%%%%%%%%%%%%%ask chris%%%%%%%%%%%%%%%%
%is sticker in good place? 


%%%%%%%%%%%%%%%%to do%%%%%%%%%%%%%%%%%%%%%%%%%%%

%1add in intro a bullet list of objectives or planned contributions of the project & in conlucion add a bulleted list of objects that have been achieved

%2restricted list and define it  (rr lanagueg)

%3don't restrict the driving expression (check syntax) 

%4say special thing about hd and tl is they only take one value (not that they are built-in)

%5discussion of the steps to be taken in the design (detailed and about their order, is this the only one etc) 

%6fix parser for nested where loops 

%7add endwhere to syntax 

%8expalin the syntax 

%9add new syntax (from chris)

%10You already have "is it correct"?  (which begs the question - "what does it mean to be correct?") but you haven't asked "is it complete?" - i.e. are there any examples of recurrence-relation model definitions that are correct with resect to the syntax that cannot be correctly translated?  And how can you be sure?
...
%136.3 autamed (new parse tree)

%14code and make sure it works 
...
Though the translation can be seen to produce an InterDyne simulation, the question still remains about the correctness of the steps in this process. Here two arguments for the correctness of meaning of this approach are be presented.  
...
The second argument relates to the functionality of each step, if each step can produce the same outputs given the same inputs then they can be seen as numerically equivalent. To test the functionality, each step was converted into a Miranda program; this conversion was done using a program  (this program is shown in Appendix A) to reformat the target states of each step into their Miranda equivalent, the produced programs were then edited to run in the Miranda compiler, with internal functions and types defined. These programs were also edited to allow for any time step to be asked for instead of just $3$ as shown in the design process, these programs were then grouped into a single program  (this program is shown in Appendix A) and executed for five different time steps (0, 1, 2, 3 and 4). Table~\ref{tbl:meanfield_compare_to_Ising} shows the results from these runs, as can been seen all steps produced the same output. This shows, at least for the values tested, the numerical equivalence between each step of the translation and hence between the initial recurrence relation model and the final agent-based model. 
\begin{table}[H]
