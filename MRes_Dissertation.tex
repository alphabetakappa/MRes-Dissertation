...
% in section 5.5 your code has thd3.hd, why is this hd there? you have already headed the inputs list hence returning a list of the wrappers for that time step. So why head again? is this because each set of messages are in their own list?
...
%line (is this right? (checker))

%some questions in paper under help 

%should I add the translation in appendex (its going to be long) (and how should i lay iit out)

%should I add the mirand version in appendex? 
...
%2rewrite conclusion x2 the size

%%%%%%%%%%finish%%%%%%%%%%%%%

%3write conversion code. Lexer, parser, first step
...
\begin{abstract}
\noindent {\it 
This dissertation investigates a process to increase the acceptance of agent-based models by relation from recurrence %I couldnt read the writing so this is wrong (help)
relation models. The work presented in this dissertation is designed to combat two main aspects that hinder the up take in agent-based models: a lack of a model-wide formalism, and a lack of familiarity to those unaccustomed to the technique. 
...
The process presented in this dissertation is to create an agent-based model equivalent to recurrence relation model. Recurrence relation models are both widely accepted and have a model-wide formalisation. Creating an equivalence between these two models allows for the recurrence relation model to give the agent-based model both a formalisation and a familiar representation, for economists.       
...
%This dissertation seeks to increase the acceptance of agent-based models within the economics community by creating a solution to the two major factors holding these models back, formalisation and familiarity. %you said repetitive I think it is needed as I haven't mentioned formalisation and familiarity as a down side yet  (help)
...
The translation process in this dissertation takes a step-by-step approach, with each step gradually translating the model. This allows for these issues to be tackled in a more modular fashion. This also makes the equivalence of the two models more apparent, if each small step is equivalent then so too must be the models, assuming that the transitivity of equivalence. 
...
\end{figure} 
Feedback loops can be encompass multiple entities, in Fig.~\ref{fig:exampletwofeedback} a feedback loop is shown that encloses two different entities. An output from $E2$ is passed to $E3$ which in turn creates a new input for $E2$; though $E2$ input is not directly its own output, it does depend upon it. 
\begin{figure}[H]
	\centering
...
A formal mathematic definition of the whole system being model is given when using recurrence relations due to three factors: the way in which they model the whole system as a entity, the static view point they give to the system and the mathematical stye which they take~\cite{rrformulism}.  %is this right?(checker)
...
Their work shows the ability to prefer estimations on agent-based models, counter to some criticism that has been levelled at agent-based models. Some difficulty in this approach can come from agent-based models with complicated algorithmic behaviour within the agents, specifying these models as difference equations can be challenging task. The work in this dissertation does not directly suffer from this problem as all systems are first described as recurrence relations before being translated into agent-based models, meaning that the equation view will only be as complex as the original writer intends.  
...
\begin{figure}[H]
	\centering
	\lstinputlisting[language=Miranda]{BNF.m}
...
%in fruther work
%implimitation of delays as explained here, is correct presevers the meaning of the origin, but for large simulations might have a high over head, due to the number of agents being created and messages.
%a final step would be to convert from this to the Interdyne system, where these agentas are incoperated in the simularot harness 
