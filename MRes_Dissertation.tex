...
%move emergence into its own section
...
%say that differnce equations are top-down vs ABM that are bottom up

\begin{document} 
\title{A Conversion of both Semantics and Representation between Agent-Based Models and Recurrence Relations}
...
\begin{abstract}
\noindent {\it 
This paper investigates a method of describing interacting systems from two opposing view points, recurrence relations and agent-baed models. These two methods take fundamentally different approaches with recurrence relations being top-down, and agent-based models being bottom-up.\\
Connecting these two methods allows for a more complete investigation of emergent behaviour occurring within interacting systems. Agent-based models offer an attractive way of analysing emergent behaviour, with the ability to investigate individual interactions throughout a simulation. Agent-based models however tend to be less well understood and accepted by non-computer science expertise, this is in contrast to recurrence relation which are normally well understood. Creating a correctness preserving link between recurrence relations and agent-based models allows for simulations to be understood in their recurrence relation representation and hence have their agent-based model representation accepted. This is important in fields such as finance as it opens up new tools for economists and regulators to use in understanding emergence in complex markets.\\
...
%each RR now sends its data as a tuple (to, from, data)
%only sends the information that is needed (this doesnt allways need to be needed just some times is enough)
%change logic to read this new format
%might need to send multiple copies of a message to different wrappers, each recipient needs their own version now there is a to field that is required


\subsection{Message Selection}
%the harness now only gives an wrapper messages that are addressed to it


\subsection{Broadcast Messages}
%if multiple versions of the same message are constantly being sent to the same wrappers, turn these into broadcast messages 
